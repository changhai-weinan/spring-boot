 ...
 * </pre>
 * <p>
 * {@link SpringApplication}s 可以从各种不同的源读取bean.它是
 * 通常建议使用一个{@code @Configuration}类来引导应用,
 * 但是,您也可以从以下方式设置{@link #getSources() sources}:
...
public class SpringApplication {
	...
	/**
	 * 应用启动,创建并刷新容器{@link ApplicationContext}.
	...
	public ConfigurableApplicationContext run(String... args) {
		...
		// 系统属性java.awt.headless设置,默认true
		...
		try {
			...
			// 根据webApplicationType创建容器,默认创建的容器实例类型为AnnotationConfigServletWebServerApplicationContext(反射)
			...
			// ApplicationRunner与CommandLineRunner实例Bean调用
		...
		try {
			// 发布ApplicationReadyEvent事件,Ready表示准备完毕的意思
	 ...
	 * @param type class
	 * @param <T>  泛型
	 * @return Collections of class instance
	 */
	...
	private void callRunners(ApplicationContext context, ApplicationArguments args) {
		// 获取容器中对应类型的Runner Bean
		List<Object> runners = new ArrayList<>();
		runners.addAll(context.getBeansOfType(ApplicationRunner.class).values());
		runners.addAll(context.getBeansOfType(CommandLineRunner.class).values());

		// runner排序(实现Ordered接口或使用@Order注解)
